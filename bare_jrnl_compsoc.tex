...
%
% Note from George: When writing in LaTeX, It is best to put one sentence per line. The reason 
% is that LaTeX is smart enough to know that a blank line is the end of paragraph. It also makes
% it easy to see changes in sentence wording, etc.
% 
% In our case, we also need to get things in the right order. So by using one sentence per
% line, you can easily move the sentence to the right place (or even comment it out).
%

% Saved here just in case...
% This all seems like detail for the paper, not for the abstract.
%Nevertheless, the extraordinarily high parallel performance requirements demanded by ongoing applications are channeling development efforts towards very acute computational perspectives.
%An example of this trend is  deep convolutional neural networks, which employ dense matrix computations.
%Such computations lend themselves to efficient computation on \gls{gpu} or even neuromorphic chips, which are increasingly prevalent in emerging supercomputers as well.
%While this specialization trend has improved parallelization efficiency considerably, it has imposed important limitations to alternative, biologically-inspired designs that take a real advantage of such parallelism.
%and thus are affected by such restrictions, since their algorithms do not necessarily use SIMD-style matrix multiplications and hence data coalescence is seriously impaired in optimized chips.
%Furthermore, when constructing our model, the parallelization strategy has to be robust enough so that it is not affected by eventual algorithmic modifications.
%In this paper we introduce a hybrid \gls{mpi} and \gls{omp} parallelization scheme to acclerate and scale our computational model.
%Our model is directly inspired by the neuroanatomy and physiology of the mammalian cortical tissue.

\IEEEtitleabstractindextext{%
\begin{abstract}
The interdisciplinary field of neuroscience has made significant progress in recent decades, providing the scientific community in general with a new level of understanding on how the brain works beyond the store-and-fire model found in traditional neural networks.
Meanwhile, \gls{ml} based on established models has seen a surge of interest in the \gls{hpc} community, especially through the use of high-end accelerators, such as \gls{gpu}s, including \gls{hpc} clusters of same.
In our work, we are motivated to exploit these high-performance computing developments and understand the scaling challenges for new--biologically inspired--learning models on leadership-class HPC resources.
These emerging models feature sparse and random connectivity profiles that map to more loosely-coupled parallel architectures with a large number of cores per node.
Contrasted with traditional \gls{ml} codes, these methods exploit loosely-coupled sparse data structures as opposed to tightly-coupled dense matrix computations, which benefit from SIMD-style parallelism found on \gls{gpu}s.
In this paper we introduce a hybrid \gls{mpi} and \gls{omp} parallelization scheme to accelerate and scale our computational model based on the dynamic of cortical tissue.
We ran computational tests on a leadership class visualization and analysis cluster at Argonne National Laboratory.
We include a study of strong and weak scaling, where we obtained parallel efficiency measures with a minimum above 87\% and a maximum above 97\% for simulations of our biologically inspired neural network on up to 64 computing nodes running 8 threads each.
This study shows promise of the MPI+OpenMP hybrid approach to support flexible and biologically-inspired computational experimental scenarios.
In this work we also show the viability in the application of these strategies in high-end leadership computers in the future.
\end{abstract}

% Note that keywords are not normally used for peerreview papers.
\begin{IEEEkeywords}
\gls{mpi}, \gls{omp}, biologically-inspired computational models, neuroscience, irregular computation, sparse computation
\end{IEEEkeywords}}
...
\begin{figure*}[ht]
    ...
    %\caption{\gls{cstm} representation with a connection scheme for a cortical column in the Encoder Layer. Each cylinder in the \gls{el} and in the \gls{cl} represents a \gls{cc} in neural tissue. Each prism in the \gls{mrstsa} represents a real valued variable. This is a visualization of a \gls{cc} (in red) and its three receptive fields (in yellow). The receptive field of a \gls{cc} is an array that defines a set of \glspl{cc} with which such column could be connected. The receptive field of a \gls{cc} on the \gls{mrstsa} determines an array of real valued variables with which such column could be connected. A subset of \glspl{cc} in a receptive field (in green) represents the \glspl{cc} that are really connected with the \gls{cc} in red. A similar scenario could be described for the green prisms on the \gls{mrstsa}. The size, wrap-around property and percentage of established links (in green) inside a receptive field are tunable parameters for the model. In this work, only lateral connections have been implemented since in the current implementation there are no upper cortical layers from which to bring apical connections.}
...
%\section{Our Computational Model and Implementation}

%{\bf Absolutely need a proper paragraph here. Can this be driven by one of the figure captions?}

%{\bf Important suggestion from Silvio: We absolutely must make sure this is in the right order. That is, it should follow the dataflow. It is also important that we highlight the Encoder Layer and deemphasize other other parts. Corpora generation is a preprocessing step. We need a good (non-incriminating) explanation for why we're not addressing parallel performance of other layers, e.g. Cortical}
...
%{\bf You need to describe the details of vectorization, optimization, etc. What compiler is used? What options? }

%{\bf MPI version, OpenMP version / options, Vectorization info for GCC. Is LLVM being used? }
...
\\
  ...
  \subfloat[Efficiency vs. the number of \glspl{cpu}.\label{sub:Corpora_Weak_Scaling_Efficiency}]{%
	...
	%\caption{Strong and Weak scaling test of Corpora Generation. Each Python \gls{mpi} rank runs in a different \gls{cpu}. Since Cooley nodes have 12 \glspl{cpu} we had to ask for 2 nodes in order to run on 16 \glspl{cpu}, 3 nodes in order to run on 32 \glspl{cpu} and 6 nodes in order to run on 64 \glspl{cpu}.}
